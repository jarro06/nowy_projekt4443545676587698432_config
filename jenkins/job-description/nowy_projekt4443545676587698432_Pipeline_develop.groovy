...
def job_config = [
    job: [
        name: "nowy_projekt4443545676587698432_Pipeline_develop"
    ],
    git: [ 
        branch: "develop"
    ]
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                    jslGit.checkout( config, "JEQP", "nowy_projekt4443545676587698432_Pipeline_develop", "develop")
                }
            }    
        }
        stage('Build') {
            steps {
                echo "Build"
                script {
                    dir ("nowy_projekt4443545676587698432") {
                        jslMaven.build()
                    }
                }
            }    
